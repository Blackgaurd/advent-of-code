...
paths = list(map(lambda x: list(map(lambda y: [y[0], y[1] - min_x + 2 * (max_x - min_x + 1)], x)), paths))
cave = [[0 for _ in range(5 * (max_x - min_x + 1))] for _ in range(max_y + 3)]
...
cave[-1] = [1 for _ in range(len(cave[-1]))]
